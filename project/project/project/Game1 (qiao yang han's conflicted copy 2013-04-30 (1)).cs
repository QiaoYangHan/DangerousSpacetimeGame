    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
                ...
                || gamestate == GameStates.CP1RP2)
            {
                ...
                for (int i = 0; i < ets.Count; i++)
                {
                    if (i >= 0)
                    {
                        ...
                        else if (ship.laser.shouldUpdate && ship.laser.laserRect.Intersects(eRect))
                        {
                            ets.RemoveAt(i);
                            i--;
                            score++;
        ...
        protected override void Draw(GameTime gameTime)
        {
                ...
                || gamestate == GameStates.TP1RP2 || gamestate == GameStates.SL)
            {
                ...
                {
                    spriteBatch.DrawString(courierNewBiger, "GAME OVER", new Vector2(950, 475), Color.Yellow);
                }
                foreach (Flag flag in flags)
                {
